...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from product where id='$id' $condtion1");
...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($con,"select * from product where name='$name'  $condtion1");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				...
				move_uploaded_file($_FILES['image']['tmp_name'],'../media/product_images/'.$image);
									...
									<div class="row"  id="image_box">
									  <div class="col-lg-10">
									   <label for="categories" class=" form-control-label">Image</label>
										...
										<?php
										if($image!=''){
											echo "<a target='_blank' href='".PRODUCT_MULTIPLE_IMAGE_SITE_PATH.$image."'><img width='150px' src='".PRODUCT_MULTIPLE_IMAGE_SITE_PATH.$image."'/></a>";
										}
										?>
									  </div>
									  <div class="col-lg-2">
